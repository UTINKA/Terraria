...
namespace Terraria.GameContent.Shaders
{
  public class WaterShaderData : ScreenShaderData
  {
    ...
    public WaterShaderData(string passName)
      : base(passName)
    {
      for (int index = 0; index < this._waveMaskChain.Length; ++index)
        this._waveMaskChain[index] = new Texture2D(Main.instance.GraphicsDevice, 200, 200, false, SurfaceFormat.Color);
    ...
    public override void Apply()
    {
      ...
      Vector2 vector2 = (Main.drawToScreen ? Vector2.Zero : new Vector2((float) Main.offScreenRange, (float) Main.offScreenRange)) - Main.screenPosition;
      ...
      this.UseTargetPosition(Main.screenPosition - Main.sceneWaterPos + new Vector2((float) Main.offScreenRange, (float) Main.offScreenRange));
      this.UseImageOffset(-(vector2 * 0.25f - this._lastDistortionDrawOffset) / new Vector2((float) this._distortionTarget.Width, (float) this._distortionTarget.Height));
      base.Apply();
    }
