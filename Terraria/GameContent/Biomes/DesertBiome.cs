  ...
  public class DesertBiome : MicroBiome
  {
    private void PlaceSand(DesertBiome.ClusterGroup clusters, Point start, Vector2 scale)
    {
      ...
      for (int index1 = -num3; index1 < num1 + num3; ++index1)
      {
        ...
        int min = val1 - (int) ((double) num8 * (double) ((float) val1 - num5)) + num6;
        int val2 = val1 - (int) ((double) ((float) val1 - num5) * ((double) num8 - 0.150000005960464 / Math.Sqrt(Math.Max(0.01, (double) Math.Abs(8f * num7) - 0.1)) + 0.25));
        int num9 = Math.Min(val1, val2);
        int num10 = (int) (70.0 - (double) Utils.SmoothStep(0.5f, 0.8f, Math.Abs(num7)) * 70.0);
        if (min - (int) numArray[index1 + num3] < num10)
        {
          for (int index2 = 0; index2 < num10; ++index2)
          {
            int index3 = index1 + start.X;
            int index4 = index2 + min - 70;
            GenBase._tiles[index3, index4].active(false);
            GenBase._tiles[index3, index4].wall = (byte) 0;
          }
          numArray[index1 + num3] = (short) Utils.Clamp<int>(num10 + min - 70, min, (int) numArray[index1 + num3]);
        }
        for (int index2 = val1 - 1; index2 >= min; --index2)
        {
