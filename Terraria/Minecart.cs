	...
	public static class Minecart
	{
		...
		public static void Initialize()
		{
			...
			for (int num32 = 0; num32 < Minecart.trackType.Length; num32++)
			{
				...
				case 2:
					...
					{
						if (Minecart.firstRightBoostFrame == -1)
						{
		...
		public static BitsByte TrackCollision(ref Vector2 Position, ref Vector2 Velocity, ref Vector2 lastBoost, int Width, int Height, bool followDown, bool followUp, int fallStart, bool trackOnly)
		{
			...
			while (true)
			{
							...
							else
							{
								if (num12 == -3)
								{
									if (trackState == Minecart.TrackState.OnFront)
									{
										...
										{
											if (Minecart.rightSideConnection[num10] == 2)
											{
												matrix = Matrix.CreateRotationZ(0.7853982f);
											}
											else
								...
								else
								{
									...
									{
										if (trackState4 == Minecart.TrackState.AboveTrack)
										{
											if (trackState3 == Minecart.TrackState.OnTrack)
											{
											...
											{
												if (trackState3 == Minecart.TrackState.BelowTrack)
												{
													trackState2 = Minecart.TrackState.AboveFront;
												}
												else
												{
													if (trackState3 == Minecart.TrackState.AboveTrack)
													{
										...
										{
											if (trackState4 == Minecart.TrackState.BelowTrack)
											{
												if (trackState3 == Minecart.TrackState.OnTrack)
												{
													trackState2 = Minecart.TrackState.OnBack;
				...
				{
					if (trackState2 == Minecart.TrackState.OnFront || trackState2 == Minecart.TrackState.OnBack || trackState2 == Minecart.TrackState.OnTrack)
					{
		...
		public static bool FrameTrack(int i, int j, bool pound, bool mute = false)
		{
			...
			if (array != null)
			{
				...
				else
				{
					...
					{
						if ((int)tile.FrontTrack() != array[num5])
						{
							flag4 = true;
						}
					}
				...
				{
					if ((int)tile.BackTrack() != array[num6])
					{
						flag4 = true;
					}
				}
