  ...
  public static class Utils
  {
    ...
    public static string[] WordwrapString(string text, DynamicSpriteFont font, int maxWidth, int maxLines, out int lineAmount)
    {
      ...
      while (stringList2.Count > 0)
      {
        ...
        else if (flag)
        {
          if (font.MeasureString(str1).X > (double) maxWidth)
          {
            ...
            while (true)
            {
              ...
              if (dynamicSpriteFont.MeasureString(str7).X <= (double) maxWidth)
              {
                string str8 = str3;
                ch = str1[startIndex++];
        ...
        else if (font.MeasureString(strArray[index1] + str1).X > (double) maxWidth)
        {
          ++index1;
          if (index1 < maxLines)
            flag = true;
    ...
    public static void PoofOfSmoke(Vector2 position)
    {
      ...
      {
        Dust dust = Main.dust[Dust.NewDust(position, 14, 14, 16, 0.0f, 0.0f, 100, (Color) null, 1.5f)];
        Vector2 vector2_1 = Vector2.op_Addition(dust.position, new Vector2(5f));
        dust.position = vector2_1;
        Vector2 vector2_2 = Vector2.op_Multiply(Vector2.op_Multiply(Vector2.op_Multiply((Main.rand.NextFloat() * 6.283185f).ToRotationVector2(), new Vector2(2f, 0.7f)), 0.7f), (float) (0.5 + 0.5 * (double) Main.rand.NextFloat()));
    ...
    public static Vector2 RotatedBy(this Vector2 spinningpoint, double radians, Vector2 center = null)
    {
      ...
      // ISSUE: explicit reference operation
      double num3 = (double) ^(float&) local1 + (vector2_1.X * (double) num1 - vector2_1.Y * (double) num2);
      // ISSUE: cast to a reference type
      // ISSUE: explicit reference operation
      ...
      // ISSUE: cast to a reference type
      // ISSUE: explicit reference operation
      double num4 = (double) ^(float&) local2 + (vector2_1.X * (double) num2 + vector2_1.Y * (double) num1);
      // ISSUE: cast to a reference type
      // ISSUE: explicit reference operation
    ...
    {
      // ISSUE: explicit reference operation
      byte num = ((Color) @color).get_R();
      string str1 = num.ToString("X2");
      // ISSUE: explicit reference operation
      num = ((Color) @color).get_G();
      string str2 = num.ToString("X2");
      // ISSUE: explicit reference operation
      num = ((Color) @color).get_B();
